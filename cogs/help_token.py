...
async def load_cogs():
    ...
    # === [PERBAIKAN] Hapus 'help_token' dari daftar ini ===
...
async def main():
    ...
    async def on_ready():
        ...
        # 3. Sinkronisasi Slash Commands (Jalankan SEKARANG)
        try:
        ...
        # 4. Cek kesehatan token_state.json
        try:
            async with bot.github_lock:
            ...
            # Tangkap error apa pun dari pengecekan state agar bot tidak crash
            logger.error(f"KRITIS: Gagal memuat token_state.json: {e}. Bot akan berjalan dengan klaim tertutup.")
